...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return _clamp_score(precision_sum / relevant_total)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "rubric below. Return JSON with one numeric 0-1 score per criterion.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}\n"
        )
        ...
        try:
            ...
            scores = {
                criterion: _clamp_score(float(raw_scores.get(criterion, 0.5)))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if position in (0, 1, "first", "A", "a"):
                ...
                other_scores.extend(numeric_scores)
        ...
        if first_scores and other_scores:
            ...
            positional_bias = first_avg > other_avg + 0.05
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if len(sorted_scores) > 1 and sorted_scores[1][1] - sorted_scores[0][1] < 0.05:
            return "Multiple issues detected â€” review full pipeline"

        lowest = sorted_scores[0][0]
        if lowest == "faithfulness":
            return "Context is missing or irrelevant â€” improve retrieval"
        if lowest == "relevance":
            return "Answer does not address the question â€” improve prompt clarity"
        return "Answer is missing key information â€” increase context window or improve generation"
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            failure_id = None
            if failure.qa_pair.metadata:
                failure_id = failure.qa_pair.metadata.get("id")
            failure_id = failure_id or f"F{index:03d}"
            suggestion = (
                suggestions[index - 1]
                if index - 1 < len(suggestions)
                else "Review trace and add a targeted fix"
            )
            lines.append(
                "| {failure_id} | {failure_type} | {root_cause} | {suggestion} | Open |".format(
                    failure_id=str(failure_id).replace("|", "/"),
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) or any(f.faithfulness < 0.5 for f in failures):
            suggestions.append("Add a grounding check that flags claims not supported by retrieved context")
        if (
            categories.get("irrelevant", 0)
            or categories.get("off_topic", 0)
            or any(f.relevance < 0.5 for f in failures)
        ):
            suggestions.append("Tighten the answer prompt to restate the user intent before generating")
        if categories.get("incomplete", 0) or any(f.completeness < 0.5 for f in failures):
            suggestions.append("Improve retrieval coverage and require the generator to include key conditions and exceptions")

        suggestions.append("Review the lowest-scoring traces and add similar cases to the golden dataset")
        suggestions.append("Track faithfulness, relevance, and completeness as CI regression gates")
